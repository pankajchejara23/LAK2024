...
This document offers information on usability evaluation of a web-based application named CoTrack. The document first discusses CoTrack which is a web-based application for conducting group activity with monitoring functionality. Next, the document provides detailed account on how the tool was evaluated with different stakeholders (e.g. Teachers, Students, Researchers) using SUS (System Usability Survey).

Let's talk a bit about CoTrack.


## CoTrack

CoTrack is a web-application which allows teachers/researchers to organize group activities for the purpose of implementing a collaborative learning activity or collecting multimodal data for research.  

CoTrack utilizes Etherpad (a collaborative text editor) to allows group's participants to write collaboratively. CoTrack tracks participant's writing contribution by processing logs from Etherpad. In addition, CoTrack can also be configured to record audio/video along with real-time processing of those data. For example, a teacher can configure CoTrack to extract participant's speaking behavior through speaking time, turn-taking and speech-to-text data. 

CoTrack makes it easy for teachers to add multimodal data tracking for monitoring their students' participation during collaborative learning activities in classrooms. Morevoer, this easy of use also enable researchers from non-techincal background to utilize the potential of multimodal data for their own research. 


### Technical Details of CoTrack
CoTrack has been developed using Django framework. It uses [Etherpad](https://etherpad.org/) to offer a collaborative writing space to group's participants. To track the participants' writing activities, CoTrack uses Etherpad's REST API to fetch writing logs. 
...
* The **Storage** module is responsible for saving multimedia files and preprocessed features. The module stores audio and video both files in WEBM format.


* The last two modules, **Visualisation** and **Prediction**, leverage the extracted features to generate real-time dashboards of students' activities and to estimate collaboration quality. Figure below shows a dashboard generated from a dataset collected from an authentic classroom setting. The dashboard provides insights into group speaking behaviour, displays speech content in the form of a word cloud, and presents writing contributions from each group. In addition, the system also offers the downloading of synchronized and anonymized multimodal data in CSV format for research purposes as well as downloading the responses from all groups into a single file for the teacher.
...
This section presents our methodology employed for the design and development of the proposed system. The presented system has been co-designed and developed over the duration of three years (2020-2022) in three major iterations involving a total of **58 teachers, 603 students and 6 researchers**. The following subsections offer a detailed description of those three iterations.
...
CoTrack tries to tackle challenges associated with multimodal data collection, e.g., data preprocessing, data synchronization, data anonymization. The following offers more information on how CoTrack addressess these various challenges.

* **Data Synchronization**
CoTrack collects audio, video and log data. The effective utilization of these data requires these data to be synchronized. To achieve this CoTrack uses [ServerDate library](https://github.com/NodeGuy/server-date). This library enables the usage of server's clock on client's side. As a consequency, all the recorded/pre-processed data share a common timestamp. This remove the need to manually synchronization of data before using it for research purposes.


* **Data Anonymization**
CoTrack performs data anonymization by replacing each user's identifier with a anonymized string. Hence, when researcher or teacher downloads the pre-processed data from CoTrack, the data contains a anonymized identifier. 


* **Data Pre-processing**
CoTrack performs data pre-processing in real-time which further enable the tool to generate a dashboard for teachers. The current version of CoTrack processes audio and log data. To process audio data, CoTrack uses Voice Activity Detection (VAD) to detect speaking activity which later used to compute speaking time and turn-taking features. CoTrack also uses Google Speech-To-Text API to obtain speech data. 


* **Data Recording**
CoTrack supports recording of audio or video data for research purposes. CoTrack performs data recording in two ways: first, it records for data in chunks (for every 5 minutes) and send it to the server; second, it also sends a final file upon completion of the activity to the server. In this way, CoTrack minimises the chance of data missing due to reasons, e.g., network delay. 

## Usability Evaluation
To assess usability of CoTrack, we utilized a widely used survey instrument (i.e., System Usability Survey). In total, two researchers (R1, R2) and two teachers (T1, T2) used the system for research and teaching purposes. In addition of obtaining response of teachers/researchers, we also collected responses from students from the classroom where teacher (T1) used the system. 


> We want to emphasize that the system has been used by a total of **58 teachers, 603 students and 6 researchers**. However, this study only present the responses from 2 teachers, 2 researchers, and 24 students. We are still in process of collecting and processing survey's responses from other users of CoTrack.
...
There were more male (N=21) participants than female (N=7). Surprisingly, the scores were same for both gender. 

![](./gender.png)

**Difference in score based on prior experience**

The students with prior experience of using CoTrack gave it a higher score compared to those who used it first time. This could be due to learning curve associated with the use of CoTrack. Though, it is designed to be used easily it still requires participants to be familiar with its functioning.
![](use_before.png)
